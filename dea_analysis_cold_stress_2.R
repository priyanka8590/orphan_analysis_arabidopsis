...
setwd("~/Box/analysis_orphan_gene_aim_3/deseq_analysis/cold_stress/Exp2_RPS4_HS")
...
salmon_counts_pi_def <- salmon_counts %>% select(Gene, starts_with('SRR34035'))
...
dea_exp1_anno <- filter(metadata_table, study_accession %in% 'SRP073511')
...
dea_exp1_anno <- dea_exp1_anno[,c("genotype", "treatment")]
...
dds <- DESeqDataSetFromMatrix(countData = salmon_counts_pi_def_mat, colData = dea_exp1_anno, design = ~ genotype + treatment + genotype:treatment)
...
res <- results(dds, contrast = list(c("genotype_lhp1_vs_ebs.shl", "genotype_ebs.shl.lhp1_vs_ebs.shl")))
resUpregulated <- subset(res, log2FoldChange > 0 & padj < 0.05)
resDownregulated <- subset(res, log2FoldChange < 0 & padj < 0.05)
write.table(resUpregulated, file = "upregulated_in_lhp1_vsebs.shl.lhp1.tsv", sep = "\t", quote = F, row.names = TRUE)
write.table(resDownregulated, file = "downregulated_in_lhp1_vsebs.shl.lhp1.tsv", sep = "\t", quote = F, row.names = TRUE)

res <- results(dds, contrast = list(c("genotype_lhp1_vs_ebs.shl", "genotype_emf1_vs_ebs.shl")))
resUpregulated <- subset(res, log2FoldChange > 0 & padj < 0.05)
resDownregulated <- subset(res, log2FoldChange < 0 & padj < 0.05)
write.table(resUpregulated, file = "upregulated_in_lhp1_vs_ebs.shl.tsv", sep = "\t", quote = F, row.names = TRUE)
write.table(resDownregulated, file = "downregulated_in_lhp1_vs_ebs.shl.tsv", sep = "\t", quote = F, row.names = TRUE)

res <- results(dds, contrast = list(c("genotype_wild.type_vs_ebs.shl", "genotype_emf1_vs_ebs.shl")))
resUpregulated <- subset(res, log2FoldChange > 0 & padj < 0.05)
resDownregulated <- subset(res, log2FoldChange < 0 & padj < 0.05)
write.table(resUpregulated, file = "upregulated_in_WT_vs_emf1", sep = "\t", quote = F, row.names = TRUE)
write.table(resDownregulated, file = "downregulated_in_WT_vs_emf1.tsv", sep = "\t", quote = F, row.names = TRUE)

res <- results(dds, contrast = list(c("genotype_ebs.shl.lhp1_vs_ebs.shl", "genotype_emf1_vs_ebs.shl")))
resUpregulated <- subset(res, log2FoldChange > 0 & padj < 0.05)
resDownregulated <- subset(res, log2FoldChange < 0 & padj < 0.05)
