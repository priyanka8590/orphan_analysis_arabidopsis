...
salmon_counts_pi_def <- salmon_counts %>% select(Gene, starts_with('SRR32234'))
...
dea_exp1_anno <- filter(metadata_table, study_accession %in% 'SRR72277')
dea_exp1_anno <- read.delim("metadata_table_cold_stress_1")
dea_exp1_anno <- as.data.frame(dea_exp1_anno)

dea_exp1_anno<-mutate(dea_exp1_anno,genotype=sapply(strsplit(dea_exp1_anno$experiment_title, split=' P', fixed=TRUE),function(x) (x[1])))
dea_exp1_anno$treatment <- c("P-", "P-", "P-", "P-", "P-", "P-", "P+", "P+", "P-", "P-", "P+", "P+")
