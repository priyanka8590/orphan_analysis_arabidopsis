...
setwd("~/Box/analysis_orphan_gene_aim_3/deseq_analysis/Nutrient_stress")
...
#To get metadata
library(dplyr)
dea_exp1_anno <- filter(metadata_table, study_accession %in% 'SRP068145')
...
dea_exp1_anno <- dea_exp1_anno[,c("treatment", "tissue")]
...
dea_exp1_anno$tissue <- factor(dea_exp1_anno$tissue)
dea_exp1_anno$treatment <- factor(dea_exp1_anno$treatment)
dds <- DESeqDataSetFromMatrix(countData = salmon_counts_pi_def_mat, colData = dea_exp1_anno, design = ~ treatment + tissue + treatment:tissue)
...
res <- results(dds, contrast = list(c("treatment_Phosphate.sufficiency_vs_Phosphate.deficiency", "treatmentPhosphate.sufficiency.tissueshoot")))
resUpregulated <- subset(res, log2FoldChange > 0 & padj < 0.05)
resDownregulated <- subset(res, log2FoldChange < 0 & padj < 0.05)
write.table(resUpregulated, file = "upregulated_in_phosphate_suffciency_shoot.tsv", sep = "\t", quote = F, row.names = TRUE)
write.table(resDownregulated, file = "downregulated_in_phosphate_suffciency_shoot.tsv", sep = "\t", quote = F, row.names = TRUE)

res <- results(dds, contrast = c("tissue", "shoot", "root"))
resUpregulated <- subset(res, log2FoldChange > 0 & padj < 0.05)
resDownregulated <- subset(res, log2FoldChange < 0 & padj < 0.05)
write.table(resUpregulated, file = "upregulated_in_root_vs_shoot.tsv", sep = "\t", quote = F, row.names = TRUE)
write.table(resDownregulated, file = "downregulated_in_root_vs_shoot.tsv", sep = "\t", quote = F, row.names = TRUE)

res <- results(dds, contrast = list(c("tissue_shoot_vs_root", "treatmentPhosphate.sufficiency.tissueshoot")))
resUpregulated <- subset(res, log2FoldChange > 0 & padj < 0.05)
resDownregulated <- subset(res, log2FoldChange < 0 & padj < 0.05)
write.table(resUpregulated, file = "upregulated_in_root_vs_shoot_with_treatment.tsv", sep = "\t", quote = F, row.names = TRUE)
write.table(resDownregulated, file = "downregulated_in_root_vs_shoot_with_treatment.tsv", sep = "\t", quote = F, row.names = TRUE)

res = results(dds, name = "treatmentPhosphate.sufficiency.tissueshoot")
resUpregulated <- subset(res, log2FoldChange > 0 & padj < 0.05)
resDownregulated <- subset(res, log2FoldChange < 0 & padj < 0.05)
write.table(resUpregulated, file = "upregulated_in_interaction_term.tsv", sep = "\t", quote = F, row.names = TRUE)
write.table(resDownregulated, file = "downregulated_in_interaction_term.tsv", sep = "\t", quote = F, row.names = TRUE)
